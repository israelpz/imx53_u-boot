...
void board_mmu_init(void)
{
	...
	*/
	...
	X_ARM_MMU_SECTION(0xF7F, 0x020, 0x001,
			...
			ARM_ACCESS_PERM_RW_NONE); /* NAND Flash buffer */
	X_ARM_MMU_SECTION(0xF7F, 0x030, 0x001,
			ARM_CACHEABLE, ARM_BUFFERABLE,
			ARM_ACCESS_PERM_RW_RO); /* NAND Flash buffer */
...
*/
	...
	/* Contrast: Configure as GPIO  */
	mxc_request_iomux(MX53_PIN_GPIO_1, IOMUX_CONFIG_ALT1); 
	/* Contrast: Configure as output */ 
	reg = readl(GPIO1_BASE_ADDR + 0x4); 
	reg |= 0x00000002; 
	writel(reg, GPIO1_BASE_ADDR + 0x4); 

	/* Backlight: Configure as GPIO  */        
	mxc_request_iomux(MX53_PIN_ATA_INTRQ, IOMUX_CONFIG_ALT1); 
	/* Backlight: Configure as output */ 
	reg = readl(GPIO7_BASE_ADDR + 0x4); 
	reg |= 0x00000004; 
	writel(reg, GPIO7_BASE_ADDR + 0x4); 
}
